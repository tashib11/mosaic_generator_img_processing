...
def find_best_match(target_segment, tile_images, weights=(0.5, 0.5)):
    ...
    target_edge_strength = extract_edge_strengths(cv2.cvtColor(target_segment, cv2.COLOR_BGR2GRAY))
    ...
    for index, tile in enumerate(tile_images):
        ...
        tile_edge_strength = extract_edge_strengths(cv2.cvtColor(tile, cv2.COLOR_BGR2GRAY))
        
        color_diff = color_difference(target_avg_color, tile_avg_color)
        edge_diff = edge_similarity(target_edge_strength, tile_edge_strength)
        
        score = weights[0] * color_diff + weights[1] * edge_diff
